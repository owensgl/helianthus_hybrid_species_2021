...
data %>%
  ...
  ylab("Site count")
ggsave("plots/gowens22.mappable.variant.snps.filtered.dp4.missing80_BBAA.des.v1.pdf",width=12,height=4)

twisst <- read_tsv("../twisst/ann.petfal.des.weights.txt",comment="#",
                   col_names=c("ann_pet","ann_des","pet_des"),skip=4)
tree_ids <- read_tsv("../vcf/gowens22.mappable.dp4.missing80.10.2kb.physical.treenames.trees.txt",
                     col_names=c("tree_id","tree"))

chisq.test(c(1043,700))$p.value
twisst %>%
  cbind(tree_ids) %>%
  select(-tree) %>%
  ...
  ggplot(.,aes(x=fct_relevel(topology, c("pet_des","ann_des","ann_pet")),y=weight_count,fill=topology)) +
  geom_bar(stat="identity") +
  theme_cowplot() +
  xlab("") +
...
ggsave("plots/ann.petfal.des.weights.twisst.v1.pdf",width=12,height=4)
...
test_blt %>%
  ...
  ylab("Genetic\ndistance") +
  xlab("")
ggsave("plots/gowens22.mappable.dp4.missing80.10.2kb.physical.subsample.des.blt.pdf")
