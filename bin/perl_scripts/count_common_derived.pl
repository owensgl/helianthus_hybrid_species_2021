...
my %counts;
my $site_counter = 1;
print "chr\tpos\tsample\tcommon_derived_present";
while(<STDIN>){
  chomp;
  my @a = split(/\t/,$_);
  if ($_ =~ /^##/){next;}
  if ($_ =~ /^#/){
    ...
    foreach my $derived_allele (sort keys %derived_alleles){
      ...
      if ($derived_count >= $minimum_derived_count){
        ...
        foreach my $i (9..$#a){
          if($outgroups{$sample{$i}}){next;}
          if ($called_genotype{$sample{$i}}){
            if ($calls{$sample{$i}}{$derived_allele}){
              my $t = $calls{$sample{$i}}{$derived_allele}/2;
              print "\n$a[0]\t$a[1]\t$sample{$i}\t$t";
            }else{
              print "\n$a[0]\t$a[1]\t$sample{$i}\t0";
            }
